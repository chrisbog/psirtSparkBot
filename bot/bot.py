...
commands = {
    ...
    "/iosxe": "Provide vulnerability list of iosxe",
    "/help": "Get help."
}
...
def process_incoming_message(post_data):
    ...
    elif command in ["/iosxe"]:
        reply = send_os_vulnerability("iosxe",message)
...
def send_os_vulnerability(ostype,incoming):


    if ostype == "iosxe":
        version = extract_message("/iosxe ", incoming.text)
    else:
        return "Unsupported IOS Type"

    sys.stderr.write("I'm receiving vulnerabilities on" + version)

    client_id = os.environ.get("CLIENT_ID")
    client_secret = os.environ.get("CLIENT_SECRET")

    sys.stderr.write("Client ID:" + client_id+"\n")
    sys.stderr.write("Client Secret:" + client_secret+"\n")

    access_token = psirt.psirtquery.get_oauth_token(client_id, client_secret)

    sys.stderr.write("Access Token:" + access_token+"\n")
    sys.stderr.write("ostype = "+ostype+"\n")
    sys.stderr.write("version = "+version+"\n")

    response = psirt.psirtquery.get_ios_vulnerability(access_token,ostype,version)
